//
//  OrderDropOffViewController.swift
//  CleanSwipe0-1
//
...
class OrderDropOffViewController: UIViewController {
    ...
    @IBAction func confirmDropoff(_ sender: AnyObject) {
        
        let orderId = appointment.transactionId
        AppointmentManager.sharedManager.selectedAppointment = appointment
        
        // Update appointment in order table
        AppointmentManager.sharedManager.updateOrderStatus(currentEmp.empId, order_id:orderId, newStatus: "delivered")
        
        // Post notification
        NotificationCenter.default.post(name: Notification.Name(rawValue: "Status Updated"), object: self)
        
        // Upload confirmed apt
        AppointmentManager.sharedManager.uploadConfirmedAppointment(appointment)
        ...
        if isRegistered {
            ...
            manager.postPushForAppointmentUpdate(appointment.customerId, message: "Awesome news!\nCleanSwiper \(currentEmp.fullName) has successfully returned your clean order! Open the app to review your service.", token: userToken)
        }else{
