...
@objc public class DateTimePicker: UIView {
    ...
    var morningButton = UIButton(type: .system)
    var afternoonButton = UIButton(type: .system)
    var eveningButton = UIButton(type: .system)
    var timeHasBeenSelected : Bool = false
    ...
    let csGreen = UIColor(red: 27.0/255.0, green: 205.0/255.0, blue: 137.0/255.0, alpha: 1.0)
    
    public var highlightColor = UIColor(red: 27.0/255.0, green: 205.0/255.0, blue: 137.0/255.0, alpha: 1) {
        didSet {
    ...
    public var dateFormat = "MMM dd, yyyy" {
        didSet {
            resetDateTitle()
        }
    }
    ...
    public var doneButtonTitle = "Schedule Pickup" {
        didSet {
    ...
    @objc open class func show(selected: Date? = nil, minimumDate: Date? = nil, maximumDate: Date? = nil) -> DateTimePicker {
        ...
        dateTimePicker.selectedDate = minimumDate!
        dateTimePicker.minimumDate = minimumDate ?? Date(timeIntervalSinceNow: 86400)
    ...
    func buttonPressed(sender: UIButton!) {
        if timeHasBeenSelected != true{
            sender.layer.borderColor = csGreen.cgColor
            sender.layer.backgroundColor = csGreen.cgColor
            sender.setTitleColor(.white, for: .normal)
            timeHasBeenSelected = true
        }else{
            
            eveningButton.backgroundColor = .clear
            eveningButton.setTitleColor(.blue, for: .normal)
            eveningButton.layer.borderColor = UIColor.black.cgColor
            afternoonButton.backgroundColor = .clear
            afternoonButton.setTitleColor(.blue, for: .normal)
            afternoonButton.layer.borderColor = UIColor.black.cgColor
            morningButton.backgroundColor = .clear
            morningButton.setTitleColor(.blue, for: .normal)
        ...
        selectedDateString = (sender.titleLabel?.text)!
    ...
    private func configureView() {
        ...
        dateTitleLabel.text = "Schedule a Pickup Window"
        dateTitleLabel.textAlignment = .center
        ...
        afternoonButton.frame = CGRect(x: (contentView.frame.width/2) - (width/3-17)/2, y: contentView.frame.height - 150, width: contentView.frame.width/3-27, height: height/4+5)
        afternoonButton.backgroundColor = .clear
        afternoonButton.layer.cornerRadius = 5
        afternoonButton.layer.borderWidth = 1
        afternoonButton.layer.borderColor = UIColor.black.cgColor
        afternoonButton.layer.masksToBounds = true
        afternoonButton.titleLabel?.lineBreakMode = NSLineBreakMode.byWordWrapping
        afternoonButton.setTitle("Afternoon\n11:00 AM\n 2:00 PM", for: .normal)
        afternoonButton.titleLabel?.textAlignment = NSTextAlignment.center
        
        
        afternoonButton.addTarget(self, action: #selector(self.buttonPressed(sender:)), for: UIControlEvents.touchUpInside)
        
        contentView.addSubview(afternoonButton)
        
        
        // Morning button
        _ = contentView.frame.width/3 - 44
        let start = (contentView.frame.width/2) - (width/3-17)/2
        let startWidth = contentView.frame.width/3-27
        morningButton.frame = CGRect(x: start-startWidth-17, y: contentView.frame.height - 150, width: contentView.frame.width/3-27, height: height/4+5)
        morningButton.titleLabel?.lineBreakMode = NSLineBreakMode.byWordWrapping
        morningButton.setTitle("Morning\n8:30 AM\n 10:00 AM", for: .normal)
        morningButton.titleLabel?.textAlignment = NSTextAlignment.center
        morningButton.backgroundColor = .clear
        morningButton.layer.cornerRadius = 5
        morningButton.layer.borderWidth = 1
        morningButton.layer.borderColor = UIColor.black.cgColor
        morningButton.layer.masksToBounds = true
        
        
        morningButton.addTarget(self, action: #selector(self.buttonPressed(sender:)), for: UIControlEvents.touchUpInside)
        
        contentView.addSubview(morningButton)
        
        // Evening button 
        
        eveningButton.frame = CGRect(x: start+startWidth+17, y: contentView.frame.height - 150, width: contentView.frame.width/3-27, height: height/4+5)
        eveningButton.layer.masksToBounds = true
        eveningButton.backgroundColor = .clear
        eveningButton.layer.cornerRadius = 5
        eveningButton.layer.borderWidth = 1
        eveningButton.layer.borderColor = UIColor.black.cgColor
        eveningButton.titleLabel?.lineBreakMode = NSLineBreakMode.byWordWrapping
        eveningButton.setTitle("Evening\n3:00 PM\n 7:00 PM", for: .normal)
        eveningButton.titleLabel?.textAlignment = NSTextAlignment.center
        
        
        eveningButton.addTarget(self, action: #selector(self.buttonPressed(sender:)), for: UIControlEvents.touchUpInside)
        
        contentView.addSubview(eveningButton)
